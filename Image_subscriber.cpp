...
float angular_window = 0.3;
float linear_window = 0.5;
...
//  dist_diff = max(dist_diff, dist_min);
  ...
  ang_diff = (320 - kalmanPoint.x)/320;
...
//  }
 ...
 // ang_diff = momentun * ang_diff + (1 - momentun) * last_pub_ang_diff;
  ...
  if (abs(dist_diff) < linear_window) {
    dist_diff = 0;
  }
  if (abs(ang_diff) < angular_window) {
    ang_diff = 0;
...
//  }
  if (last_pub_dist_diff >= 0 && dist_diff > 0) {
